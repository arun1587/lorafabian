...
// Solved here:
// http://comments.gmane.org/gmane.os.contiki.devel/19002
//

/*
void * realloc (size_t nm, size_t es){	
	return malloc(nm*es);
}
*/
void *
...
#define realloc(_x, _y) reallocation((_x),(_y))
...
CoapPDU * _CoapPDU() {
	...
	// pdu
	coap_pdu->_pdu = (uint8_t*)calloc(170,sizeof(uint8_t));
...
CoapPDU * _CoapPDU_buf3(CoapPDU *coap_pdu, uint8_t *buffer, int bufferLength, int pduLength) {
	// sanity
	if(pduLength<4&&pduLength!=0) {
		DBG("PDU cannot have a length less than 4");
	}
	...
	//if(bufferLength < coap_pdu->_bufferLength)
	//	memset(&(coap_pdu->_pdu)[bufferLength], 0, coap_pdu->_bufferLength-bufferLength);
	
	//memcpy(coap_pdu->_pdu, buffer, bufferLength);
	//free(coap_pdu->_pdu);
	memset(coap_pdu->_pdu, 0, pduLength);
	memcpy(coap_pdu->_pdu, buffer, bufferLength);
	//coap_pdu->_pdu = buffer;
	coap_pdu->_bufferLength = bufferLength;
...
CoapPDU * _CoapPDU_buf_withCPDU(CoapPDU *coap_pdu, uint8_t *pdu, int pduLength)
{
	return _CoapPDU_buf3(coap_pdu,pdu,pduLength,pduLength);
}
...
int reset(CoapPDU *coap_pdu) {
	// pdu
//	memset(coap_pdu->_pdu,0x00,coap_pdu->_bufferLength+1);
	memset(coap_pdu->_pdu,0,4);
/*
...
int validate(CoapPDU *coap_pdu) {
	if(coap_pdu->_pduLength<4) {
		DBG("PDU has to be a minimum of 4 bytes. This: %d bytes\n",coap_pdu->_pduLength);
		return 0;
	}
   ...
   // +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
	...
	while(1) {
		...
		if(bytesRemaining>0) {
			...
			if(optionHeader==0xFF) {
				...
				if(bytesRemaining>1) {
					...
					DBG("Payload found, length: %d\n",coap_pdu->_payloadLength);
					return 1;
				}
			...
			if(upperNibble==0x0F||lowerNibble==0x0F) {
				DBG("Expected option header or payload marker, got: 0x%x%x\n",upperNibble,lowerNibble);
				return 0;
			}
			DBG("Option header byte appears sane: 0x%x%x\n",upperNibble,lowerNibble);
		} else {
		...
		if(headerBytesNeeded>bytesRemaining) {
			DBG("Not enough space for extended option delta, needed %d, have %d.\n",headerBytesNeeded,bytesRemaining);
			return 0;
		}
		...
		DBG("Enough space for extended delta and length: %d, continuing.\n",headerBytesNeeded);
		...
		DBG("Got option: %d with length %d\n",optionNumber,optionValueLength);
		...
		if(optionPos + totalLength > coap_pdu->_pduLength) {
			DBG("Not enough space for option payload, needed %d, have %d.\n",(totalLength-headerBytesNeeded-1),(coap_pdu->_pduLength)-optionPos);
			return 0;
		}
		DBG("Enough space for option payload: %d %d\n",optionValueLength,(totalLength-headerBytesNeeded-1));
...
int setToken(CoapPDU *coap_pdu, uint8_t *token, uint8_t tokenLength) {
	...
	if(tokenLength==oldTokenLength) {
		DBG("Token size matches \n");
		/*	
		DBG("Token :");
		DBG_hex(&_pdu[4], 	tokenLength);
		DBG_hex(token,	tokenLength);
		*/
		memcpy((void*)&_pdu[4],token,tokenLength);
		return 0;
	}
		DBG("Token size does not match \n");
...
uint8_t* mallocPayload(CoapPDU *coap_pdu,int len) {
	...
	DBG("_bufferLength: %d, _pduLength: %d, _payloadLength: %d",
		coap_pdu->_bufferLength,coap_pdu->_pduLength,coap_pdu->_payloadLength);
...
void printHuman(CoapPDU *coap_pdu) {
	...
	for(i=0; i<coap_pdu->_numOptions; i++) {
		...
		INFOX("   Value: \"");
		int j = 0;		
		for(j=0; j<options[i].optionValueLength; j++) {
	...
	if(coap_pdu->_payloadLength==0) {
		...
		INFOX("   Value: \"");
		int j = 0;		
		for(j=0; j<coap_pdu->_payloadLength; j++) {
...
void printPDUAsCArray(CoapPDU *coap_pdu) {
	DBG("const uint8_t array[] = {\r\n   ");
	int i = 0;	
	for(i=0; i<coap_pdu->_pduLength; i++) {
		DBG("0x%.2x, ",coap_pdu->_pdu[i]);
	}
	DBG("\r\n};\r\n");
}
